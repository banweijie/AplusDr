//
//  WeSelectRecordersViewController.m
//  AplusDr
//
...
@interface WeSelectRecordersViewController (){
    UIView * view0;
    UIView * view1;
...
@implementation WeSelectRecordersViewController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(sendButton_onPress)];
...
- (void)api_message_postRecordMsg {
    ...
    if ([self.delegrate respondsToSelector:@selector(selectRecorders:examation:)]) {
        [self.delegrate selectRecorders:caseRecordsIds examation:examinationIds];
    }
    [self.navigationController popViewControllerAnimated:YES];
}
