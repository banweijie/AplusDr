...
{
    if ([selectList[path.row] isEqualToString:@"YES"]) {
        selectList[path.row] = @"NO";
        selectCount --;
    }
    else {
        if (selectCount < 3) {
    ...
    //[tv reloadRowsAtIndexPaths:[NSArray arrayWithObject:path] withRowAnimation:UITableViewRowAnimationNone];
    [tv reloadData];
...
- (void)viewDidLoad
{
    ...
    for (int i = 1; i < 30; i++) {
        ...
        int k;
        if ((k = [self.currentDoctor.workPeriod rangeOfString:[NSString stringWithFormat:@"%dA", ([the weekday] + 5) % 7 + 1]].location) != NSNotFound) {
            ...
            [typeList addObject:[NSString stringWithFormat:@"%@", [self.currentDoctor.workPeriod substringWithRange:NSMakeRange(k + 2, 1)]]];
        ...
        if ((k = [self.currentDoctor.workPeriod rangeOfString:[NSString stringWithFormat:@"%dB", ([the weekday] + 5) % 7 + 1]].location) != NSNotFound) {
            [yearList addObject:[NSString stringWithFormat:@"%d", [the year]]];
            [monthList addObject:[NSString stringWithFormat:@"%02d", [the month]]];
            [dayList addObject:[NSString stringWithFormat:@"%02d", [the day]]];
            [weekdayList addObject:[NSString stringWithFormat:@"%d", ([the weekday] + 5) % 7 + 1]];
