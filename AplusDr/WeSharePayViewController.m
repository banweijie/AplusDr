...
- (void)viewDidLoad
{
    ...
    UILabel *labe1=[[UILabel alloc]initWithFrame:CGRectMake(30, 60, 260, 80)];
    labe1.textColor=We_background_red_tableviewcell;
    labe1.font=[UIFont fontWithName:@"Menlo Regular" size:32];
    ...
    for (int i=0; i<4; i++) {
        UILabel *labe1=[[UILabel alloc]initWithFrame:CGRectMake(30, 130+30*i, 260, 30)];
        labe1.textColor=We_background_red_tableviewcell;
...
//    250
    ...
    ima.image=[UIImage imageNamed:@"supportsuccess"];
...
//    /http://test.ejren.com/patient/viewOrder.action
...
//                                     response[@"foreignObject"][@"fundingLevel"][@"funding"];//获取到众筹详情
                                     MyLog(@"====%@",response);
//                                     [self api_patient_finishOrder1:ord.foreignId];
                                 }
    ...
    [WeAppDelegate postToServerWithField:@"data" action:@"viewFunding"
                              parameters:@{
                                 ...
                                 success:^(NSDictionary * response) {
                                     ...
                                     WeFundingSupport *fund=[[WeFundingSupport alloc]initWithNSDictionary:response];
                                     MyLog(@"-----%@",fund.name);
